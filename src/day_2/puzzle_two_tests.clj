(ns day-2.puzzle-two-tests
  (:require [day-2.puzzle-two :as sut]
...
(deftest encrypted-shape-conversion
  (is (= (sut/encrypted-shape-conversion "X") :loose))
  (is (= (sut/encrypted-shape-conversion "Y") :draw))
  (is (= (sut/encrypted-shape-conversion "Z") :win)))
...
(deftest parse-paragraph-lines
  (is (= (sut/parse-paragraph-line "A Y") [:rock :rock]))
  (is (= (sut/parse-paragraph-line "B X") [:paper :rock]))
  (is (= (sut/parse-paragraph-line "C Z") [:scissors :rock])))
...
(deftest calculate-total-score
  ...
  (testing "User specific input example."
    (let [input (slurp (io/resource "day_2/input.txt"))]
      (is (= (sut/total-score input) 10560)))))
